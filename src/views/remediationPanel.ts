...
export class RemediationPanel {
    private static current: RemediationPanel | undefined;
    private constructor(
        private readonly panel: vscode.WebviewPanel,
        private readonly onApply: () => Promise<void>
    ) {
        panel.webview.onDidReceiveMessage(async (message) => {
            if (message.command === 'apply') await this.onApply();
    ...
    public static show(
        ...
        if (this.current) this.current.panel.dispose();
        const panel = vscode.window.createWebviewPanel(
            'secureGenRemediation',
            `SecureGen • ${vulnerability}`,
    ...
    private static html(vulnerability: string, explanation: string, original: string, fixed: string): string {
